export const getSearchDataApi = {
  ...
  getData: (startOffset, endOffset, codeInputArr, nameInput) => {
    const product_item_data = JSON.parse(
      localStorage.getItem(`product_item_data`)
    );

    if (!codeInputArr.length && nameInput === "") {
      return {
        data: product_item_data.slice(startOffset, endOffset),
        length: product_item_data.length,
      };
    } else {
      const result = [];

      for (let { code, name } of product_item_data) {
        if (
          (nameInput === "" || name === nameInput) &&
          (!codeInputArr.length || codeInputArr.includes(code))
        ) {
          result.push({ code, name });
        }
      }
      return {
        data: result.slice(startOffset, endOffset),
