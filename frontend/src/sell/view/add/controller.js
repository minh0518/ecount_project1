...
if (mode === 'new') {
  ...
  saveButton.addEventListener('click', () => {
    ...
    const [codeInput, nameInput] = document.querySelector('.codeInput').value.split(',');
...
if (mode === 'edit') {
  ...
  // code기반 데이터 기존 조회
  let { date = new Date(date), description, name, number, price, quantity } = getSellDataApi.getSingleDataByCode(code);
  ...
  saveButton.addEventListener('click', () => {
    ...
    const priceInput = document.querySelector('.priceInput').value;
    const descriptionInput = document.querySelector('.descriptionInput').value;
    const [_, nameInput] = document.querySelector('.codeInput').value.split(',');
