...
export const renderTable = (currentPage) => {
  ...
  dateButtons.forEach((row) => {
    row.addEventListener('click', () => {
      ...
      window.open(`../add/sellAdd.html?mode=edit&code=${codeValue}`, '_blank', 'width=1200,height=500');
    });
  });
};
...
newButton.addEventListener('click', () => {
  window.open(`../add/sellAdd.html?mode=new`, '_blank', 'width=1200,height=500');
});

const searchProductDataButton = document.querySelector('.searchProductDataButton');
searchProductDataButton.addEventListener('click', () => {
  window.open(`../../../product/view/search/productSearch.html?from=search`, '_blank', 'width=1200,height=500');
});

// 선택된 품목 삭제
const codeList = document.querySelector('.codeList');
codeList.addEventListener('click', (e) => {
  const eventDrivenNode = e.target; // 이벤트 위임
  if (eventDrivenNode.classList.contains('deleteSearchedData')) {
    eventDrivenNode.closest('div').remove();
  }
});
